...
#define DEBUG_PRINT(msg)                                                       \
  do {                                                                         \
    std::cerr << "DEBUG: " << msg << " (File: " << __FILE__                    \
...
void RDFAccumulator::accumulate(
    ...
    const std::vector<std::vector<std::string>> &pairs_input, Logger *logger) {

  // Saves the input pairs to the internal pairs vector so other methods can
  // access it
...
void RDFAccumulator::normalize(
  ...
  for (size_t p_idx = 0; p_idx < pairs.size(); ++p_idx) {
    ...
    for (size_t bin = 0; bin < num_bins; ++bin) {
      ...
      double r_hi = r_lo + dr;
      double shell_vol =
          (4.0 / 3.0) * M_PI * (std::pow(r_hi, 3) - std::pow(r_lo, 3));
...
void RDFAccumulator::write(const std::string &output_dir) const {
  ...
  for (std::size_t p = 0; p < pair_list.size(); ++p) {
    std::string filename =
        output_dir + "/rdf-" + pair_list[p][0] + "_" + pair_list[p][1] + ".dat";
...
RDFAccumulator compute_rdf(const std::vector<Frame> &frames, double dr,
...
RDFAccumulator compute_rdf(const std::vector<Frame> &frames, size_t num_bins,
                           double r_max, double box_volume,
                           std::vector<std::vector<std::string>> pairs,
                           Logger *logger) {
