...
std::string infer_format(const std::string &filename) {
  if (filename.find("XDATCAR") != std::string::npos)
    return "vasp";
...
int main(int argc, char **argv) {
  ...
  if (config.analysis.rdf.enabled) {
    ...
    // Flatten pairs for computation (unique pairs from all curves)
    std::set<std::vector<std::string>> pairs_set;
    for (const auto &curve_group : curve_sets) {
      for (const auto &curve : curve_group) {
        pairs_set.insert(curve.types);
      }
    }
    std::vector<std::vector<std::string>> pairs(pairs_set.begin(),
                                                pairs_set.end());
    ...
    // Compute RDF accumulator
    RDFAccumulator rdf_accum =
        (num_bins > 0) ? RDFAccumulator(static_cast<size_t>(num_bins), r_max)
                       : RDFAccumulator(dr, r_max);

    if (num_bins > 0) {
      rdf_accum = compute_rdf(frames, static_cast<std::size_t>(num_bins), r_max,
                              box_volume, pairs, &logger);
    } else if (dr > 0.0) {
      rdf_accum = compute_rdf(frames, dr, r_max, box_volume, pairs, &logger);
    } else {
      throw std::runtime_error(
          "Neither num_bins nor dr is properly specified in config.");
    }

    logger.arrow("Writing RDF data to " + rdf_dir + "...");
    rdf_accum.write(rdf_dir);

    // Prepare file and label groups for plotting
    std::vector<std::vector<std::string>> input_file_groups;
    std::vector<std::vector<std::string>> label_groups;
    ...
    for (const auto &curve_group : curve_sets) {
      ...
      for (const auto &curve : curve_group) {
        // Build filename from curve types like "rdf-1_2.dat"
  ...
  if (config.analysis.corr.enabled) {
    ...
    CorrAccumulator corr_accum =
        compute_correlations(config.analysis.corr, frames);
