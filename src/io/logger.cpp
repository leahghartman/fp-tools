...
Logger::~Logger() { if (out.is_open()) out.close(); }
...
void Logger::sys_info(const std::string& host,
                      const std::string& wd,
                      const std::string& ts) {
...
void Logger::rdf_info(double dr, int num_bins, double r_max, std::vector<std::vector<std::string>> pairs) {
...
void Logger::msd_info(double dt, int max_lag, const std::vector<std::vector<std::string>>& groups) {
  section("Mean Squared Displacement (MSD)");
  newline();
  kv("dt", dt, 8);
  kv("max_lag", max_lag, 8);
...
void print_progress_bar(std::size_t current,
                        std::size_t total,
                        int         bar_width,
                        ...
                        const std::string& unit_label)
{
    ...
    /* -------- toggle mouth open / closed every call ------------------ */
    static bool mouth_open = true;          // persists between calls
    std::string pacman = mouth_open ? "C" : "c";   // open vs closed
    mouth_open = !mouth_open;               // flip for next call
    ...
    const std::string eaten  = "-";   // trail behind Pac‑Man
    const std::string pellet = "·";   // uneaten pellet
    ...
    for (int i = 0; i < bar_width; ++i) {
        if (i < pos - 1)      line << eaten;        // already eaten
        else if (i == pos-1)  line << pacman;       // Pac‑Man head
        else                  line << pellet;       // uneaten dots
    }
...
void Logger::msd_summ(const std::vector<std::vector<std::string>>& groups, int num_frames, double dt, double elapsed_seconds) {
    std::ostringstream oss;

    oss << "[ MSD ] Finished in " << std::fixed << std::setprecision(2)
        << elapsed_seconds << " seconds → " << groups.size() << " group"
        << (groups.size() == 1 ? "" : "s")
        << ", " << num_frames << " frames, dt = " << dt << ".\n";
...
void Logger::rdf_summ(const std::vector<std::vector<std::string>>& pairs,
    ...
    oss << "[ RDF ] Finished in " << std::fixed << std::setprecision(2)
        ...
        << pairs.size() << " pair" << (pairs.size() == 1 ? "" : "s")
        << ", " << num_frames << " frames"
        << ", dt = " << dt;
...
void Logger::mfpt_summ(int num_radii, int num_frames, double dr, double r_max, double dt, double elapsed_seconds) {
    std::ostringstream oss;

    oss << "[ MFPT ] Finished in " << std::fixed << std::setprecision(2)
        << elapsed_seconds << " seconds → "
        << num_radii << " radii (dr = " << dr << ", r_max = " << r_max << "), "
        << num_frames << " frames, dt = " << dt << ".\n";
...
void Logger::traj_info(const InputConfig& config, const std::vector<Frame>& frames) {
    if (frames.empty()) {
    ...
    Lbox << std::fixed << std::setprecision(2) << Lx << " x " << std::fixed << std::setprecision(2) << Ly << " x " 
         << std::fixed << std::setprecision(2) << Lz;
...
void Logger::list(const std::string& label, const std::vector<std::string>& items) {
  note("  - " + label + ": ");
...
void Logger::list(const std::string& label, const std::vector<std::vector<std::string>>& list_of_lists) {
  note("  - " + label + ": ");
